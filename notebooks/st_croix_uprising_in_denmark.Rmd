...
```{r}
ggsave("graphics/stcroix_uprising_dk_bigrams.png", width = 28, height = 20, units = "cm", bg = "white")
```
...
```{r}
fireburn_dk %>% 
  filter(str_detect(fulltext_org, regex("fremkaldt oprøret", ignore_case = TRUE))) %>% 
  select(fulltext_org, timestamp, link)
```
Nr 1(3. kolonne midt)
Nr 2(2. kolonne nederst)
